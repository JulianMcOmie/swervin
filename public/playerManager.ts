...
class PlayerManager {
  ...
  constructor(scene: Phaser.Scene) {
    ...
    addPlayer(id: string, playerData: PlayerData): void {
        this.players[id] = SwerverFactory.createSwerver(this.scene, playerData.type);
        this.movePlayer(id, playerData.movementData);
    }

    addNewPlayer(id: string): void {
        const val = Math.random();
        var randomType = SwerverType.Racer;
        if (val < 0.5) {
             randomType = SwerverType.Bully;
        }

        const movementData = {x: 400, y: 300, velocityX: 0, velocityY: 0};
        let playerData = {movementData: movementData, type: randomType};
